...
def load_offer(path: str | Path) -> Offer:
    ...
    # ASSIGNMENT.md §3 mentions a rename to ``creditor_balance_cents``. The shipped
    # scaffolding, JSON cases, and smoke tests all use ``current_balance_cents``;
    # accept either so the loader is forward-compatible with the renamed field.
    balance = raw.get("current_balance_cents", raw.get("creditor_balance_cents"))
    if balance is None:
        raise KeyError("offer must define current_balance_cents (or creditor_balance_cents)")
    return Offer(
...
def offer_total_cents(offer: Offer) -> int:
    # round-half-up per ASSIGNMENT.md §3 (not Python's default banker's rounding).
    from feasibility.rounding import offer_total_cents as _otc

    return _otc(offer.settlement_pct, offer.current_balance_cents)


def program_fee_cents(offer: Offer, rules: CreditorRules) -> int:
    from feasibility.rounding import program_fee_total_cents as _pft

    return _pft(rules.program_fee_pct, offer.original_balance_cents)
